...
write.csv(answer, file = "submission_financial_services")
...
#accuracy
# predicteddata <-  data.predictions[1:100]
# obervationdata <- labeldata[3399:3498]
# str(predicteddata)
# str(obervationdata)
# summary(obervationdata)
# confusion.matrix <- table(predicteddata, obervationdata)
...
# }
# T2 <- new("dgTMatrix",
#           i = as.integer(c(0,1,2,3,3)),
#           j = as.integer(c(0,1,2,3,4)), x=c(1,1,1,1,1),Dim=4:5)
# #T2 <- as.matrix.csr(T2)
# event <- as.factor(c("E1","E2","E3","E4"))
# userid <- as.factor(c("1","2","3","1","1"))
# rownames(T2) <- event
# colnames(T2) <- userid
# uniqueuserid <- unique(userid)
# T2 <- NEWaggregate.csr(x = T2,fac = userid)
# write.matrix.csr(T2, file = "T2out.dat", y = event, fac = TRUE)
